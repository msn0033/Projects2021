    ...
    self.TotalDebit = ko.computed(function () {
        ...
        for (var i = 0, len = self.TransactionDetails().length; i < len; ++i) {
            if (self.TransactionDetails()[i].Side() === 0) {
                total +=( parseFloat(self.TransactionDetails()[i].Debit()) * parseFloat(self.TransactionDetails()[i].UsedRate()));
            }
        }
        return total;
    });
    self.TotalCredit = ko.computed(function () {
        ...
        for (var i = 0, len = self.TransactionDetails().length; i < len; ++i) {
            if (self.TransactionDetails()[i].Side() === 1) {
                total += (parseFloat(self.TransactionDetails()[i].Credit())* parseFloat(self.TransactionDetails()[i].UsedRate()));
            }
        }
        return total;
    });
